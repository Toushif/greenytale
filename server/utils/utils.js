...
console.log("VCAP", vcap.services.cloudantNoSQLDB.credentials);
...
const utils = {
    ...
    mapProductUpdateToModel: function mapProductUpdateToModel(
        product,
        request
    ) {
    ...
    removeByUserID: function removeByUserID(details, user_ID, user_ID_value) {
        ...
        while (i--) {
            if (
                ...
                arguments.length > 2 &&
                details[i][user_ID] === user_ID_value
            ) {
                details.splice(i, 1);
            }
        }
    ...
    createCloudantDB: function createCloudantDB() {
        ...
        cloudant.db
            .create("green-production")
            .then(() => {
                cloudant
                    .use("green-production")
                    .insert({ user_ID: uniqueUserID }, "Users")
                    .then((data) => {
                        console.log(data);
                    });
            })
    ...
    createCartDocument: function createCartDocument(docName, userID, products) {
        var mydb = cloudant.db.use("green-production");
